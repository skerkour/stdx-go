...
func TestChaChaKeySize(t *testing.T) {
	keySizes := []struct {
		Size int
		Err  error
	}{
	...
	for _, keySize := range keySizes {
		...
		_, err := NewCipher(nonce, key, rounds)
		if !errors.Is(err, keySize.Err) {
			t.Errorf("KeySize (%d): expected error: %v| got: %v", keySize.Size, keySize.Err, err)
			return
		}
	}
}

func TestChaChaNonceSize(t *testing.T) {
	nonceSizes := []struct {
		Size int
		Err  error
	}{
	...
	for _, nonceSize := range nonceSizes {
		nonce := make([]byte, nonceSize.Size)
		_, err := NewCipher(nonce, key, rounds)
		if !errors.Is(err, nonceSize.Err) {
			t.Errorf("NonceSize (%d): expected error: %v | got: %v", nonceSize.Size, nonceSize.Err, err)
			return
		}
	}
}
...
func TestOverflow(t *testing.T) {
	...
	for i, test := range overflowTests {
		...
		stream.SetCounterUint64(test.Counter)
