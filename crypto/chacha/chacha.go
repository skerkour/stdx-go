...
var chachaConstants = [...]uint32{0x61707865, 0x3320646e, 0x79622d32, 0x6b206574}
...
func setup(state *[64]byte, nonce, key []byte) (err error) {
	...
	case XNonceSize:
		...
		copy(tmpKey[:], key)
		hChaCha20(tmpKey[:], tmpKey[:], hNonce[:])
...
func NewCipher(nonce, key []byte, rounds int) (*Cipher, error) {
	if rounds != 20 && rounds != 12 && rounds != 8 {
		return nil, ErrInvalidNumberOfRounds
	}
...
// We use uint32 by default for compatibility reasons with /x/crypto/chacha20
// See SetCounterUint64 if you have a specific need
func (c *Cipher) SetCounter(ctr uint32) {
	if c.noncesize == INonceSize {
		binary.LittleEndian.PutUint32(c.state[48:], ctr)
	} else {
		binary.LittleEndian.PutUint64(c.state[48:], uint64(ctr))
	}
	c.off = 0
}
