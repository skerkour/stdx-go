...
func newItem[K comparable, V any](key K, value V, ttl time.Duration, enableVersionTracking bool, getHook func(K, V) (K, V)) *Item[K, V] {
	item := &Item[K, V]{
		key:     key,
		value:   value,
		ttl:     ttl,
...
func (item *Item[K, V]) Value() V {
	...
	value := item.value
	if item.getHook != nil {
		_, value = item.getHook(item.key, item.value)
	}
